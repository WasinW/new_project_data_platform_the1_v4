...
class TestStreamingConnectors(unittest.TestCase):
    """Test streaming connectors (PubSub, BigTable)"""

    def setUp(self):
        """Set up test config"""
        self.config_dict = {
            "pipeline": {
                "name": "test_streaming_connectors",
                "mode": "streaming",
                "term": "realtime"
            },
            "io": {
    ...
    @patch('dataflow_common.connectors.pubsub.ReadFromPubSub')
    def test_pubsub_read(self, mock_read_pubsub):
        """Test PubSub connector read"""
        print("\n Test: PubSub read")
        ...
        with TestPipeline() as pipeline:
            ...
            # Mock return value
            mock_read_pubsub.return_value = beam.Create([b'{"test": "message"}'])

            result = PubSubConnector.read(pipeline, subscription, "TestRead")

            mock_read_pubsub.assert_called_once_with(subscription=subscription)
            print(f"   PubSub read configured correctly")

    @patch('google.cloud.bigtable.Client')
    def test_bigtable_connector_init(self, mock_bigtable_client):
        """Test BigTable connector initialization"""
        print("\n Test: BigTable connector init")

        from dataflow_common.connectors.bigtable import BigTableConnector

        # Mock the client
        mock_client = MagicMock()
        mock_bigtable_client.return_value = mock_client
        ...
        assert connector.table_id == "test-table"

        print(f"   BigTable connector initialized correctly")


class TestConnectorHelpers(unittest.TestCase):
    """Test connector helper functions"""
    ...
    def test_connector_imports(self):
        ...
        try:
            ...
            self.fail(f"Failed to import connector: {e}")
